...
@Component({
  selector: 'tqe-nba-demo',
  templateUrl: './nba-demo.component.html',
  styleUrls: ['./nba-demo.component.scss']
})

export class NbaDemo implements OnInit {
  errorrMessage: any;
  ...
  private getData() {
    ...
    this.plumber.getNbaPlayerImpactdemoData().subscribe(
      res => {
        for (let i in res) {
      ...
      () => {
        // Get NBA table
        this.plumber.getNbademoTable().subscribe(
          win => {
            for (let i in win)
