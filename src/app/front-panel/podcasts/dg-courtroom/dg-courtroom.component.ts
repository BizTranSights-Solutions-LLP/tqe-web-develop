...
@Component({
  selector: 'app-dg-courtroom',
  templateUrl: './dg-courtroom.component.html',
  styleUrls: ['./dg-courtroom.component.css']
})
export class DgCourtroomComponent implements OnInit, OnDestroy {
  podcastData: any = [];
  ...
  ngOnInit() {
    this.podcast_subscription = this.dataService.get_all_podcasts(21,4,this.bettingOffset, this.router.url).subscribe(
      (res:any) => {
  ...
  loadMore() {
    ...
    this.dataService.get_all_podcasts(21,4,this.bettingOffset).subscribe(
      (res:any) => {
