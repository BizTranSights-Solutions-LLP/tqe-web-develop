...
export class BettingComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.loading = true;
    this.dataService.updateToolTitle.emit('Betting Tools');
    this.toolsData = this.dataService.get_tools(2, this.router.url).subscribe(
      (res:any) => {
        this.loading = false;
  ...
  openModal(url, name, id) {
    ...
    const Show = true;
    const Name = name;
    this.dataService.popupToolVideo.emit({Url , Name, Show, Id});
  }
  displayImage(index) {
