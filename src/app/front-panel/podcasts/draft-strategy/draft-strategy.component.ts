...
@Component({
  selector: 'app-draft-strategy',
  templateUrl: './draft-strategy.component.html',
  styleUrls: ['./draft-strategy.component.css']
})
export class DraftStrategyComponent implements OnInit,OnDestroy {
  podcastData: any = [];
  ...
  constructor(private dataService:DataService, private seoService: SeoService, private router: Router, private helperService: HelperService) { }

  ngOnInit() {
    this.podcast_subscription = this.dataService.get_all_podcasts(15,4,this.bettingOffset, this.router.url).subscribe(
      (res:any) => {
  ...
  loadMore() {
    ...
    this.podcast_subscription = this.dataService.get_all_podcasts(15,4,this.bettingOffset).subscribe(
      (res:any) => {
        this.bettingOffset = this.bettingOffset + 4;
        console.log(res.result.podcast_data);
