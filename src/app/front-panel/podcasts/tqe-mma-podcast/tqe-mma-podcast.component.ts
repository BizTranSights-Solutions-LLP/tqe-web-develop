...
@Component({
  selector: 'app-tqe-mma-podcast',
  templateUrl: './tqe-mma-podcast.component.html',
  styleUrls: ['./tqe-mma-podcast.component.css']
})
export class TqeMmaPodcastComponent implements OnInit, OnDestroy {
  podcastData: any = [];
  ...
  ngOnInit() {
    this.podcast_subscription = this.dataService.get_all_podcasts(20,4,this.bettingOffset, this.router.url).subscribe(
      (res:any) => {
  ...
  loadMore() {
    ...
    this.podcast_subscription = this.dataService.get_all_podcasts(20,4,this.bettingOffset).subscribe(
      (res:any) => {
