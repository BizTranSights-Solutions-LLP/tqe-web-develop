...
@Component({
  selector: 'app-dfs-powerhour',
  templateUrl: './dfs-powerhour.component.html',
  styleUrls: ['./dfs-powerhour.component.css']
})
export class DFSPowerHourComponent implements OnInit, OnDestroy {
  podcastData: any = [];
  ...
  ngOnInit() {
    this.podcast_subscription = this.dataService.get_all_podcasts(25,4,this.bettingOffset, this.router.url).subscribe(
      (res:any) => {
  ...
  loadMore() {
    ...
    this.podcast_subscription = this.dataService.get_all_podcasts(14,4,this.bettingOffset).subscribe(
      (res:any) => {
