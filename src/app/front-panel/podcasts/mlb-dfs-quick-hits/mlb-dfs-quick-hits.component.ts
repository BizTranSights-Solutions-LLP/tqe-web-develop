...
@Component({
  selector: 'app-mlb-dfs-quick-hits',
  templateUrl: './mlb-dfs-quick-hits.component.html',
  styleUrls: ['./mlb-dfs-quick-hits.component.css']
})
export class MlbDfsQuickHitsComponent implements OnInit, OnDestroy {
  podcastData: any = [];
  ...
  ngOnInit() {
    this.podcast_subscription = this.dataService.get_all_podcasts(24,4,this.bettingOffset, this.router.url).subscribe(
      (res:any) => {
  ...
  loadMore() {
    ...
    this.podcast_subscription = this.dataService.get_all_podcasts(24,4,this.bettingOffset).subscribe(
      (res:any) => {
