...
@Component({
  selector: 'app-monday-morning-pivot',
  templateUrl: './monday-morning-pivot.component.html',
  styleUrls: ['./monday-morning-pivot.component.css']
})
export class MondayMorningPivotComponent implements OnInit, OnDestroy {
  podcastData: any = [];
  ...
  ngOnInit() {
    this.podcast_subscription = this.dataService.get_all_podcasts(19,4,this.bettingOffset, this.router.url).subscribe(
      (res:any) => {
  ...
  loadMore() {
    ...
    this.podcast_subscription = this.dataService.get_all_podcasts(19,4,this.bettingOffset).subscribe(
      (res:any) => {
