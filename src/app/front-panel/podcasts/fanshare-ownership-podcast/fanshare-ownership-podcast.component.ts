...
@Component({
  selector: 'app-fanshare-ownership-podcast',
  templateUrl: './fanshare-ownership-podcast.component.html',
  styleUrls: ['./fanshare-ownership-podcast.component.css']
})
export class FanshareOwnershipPodcastComponent implements OnInit, OnDestroy {
  podcastData: any = [];
  ...
  ngOnInit() {
    this.podcast_subscription = this.dataService.get_all_podcasts(16,4,this.bettingOffset, this.router.url).subscribe(
      (res:any) => {
  ...
  loadMore() {
    ...
    this.podcast_subscription = this.dataService.get_all_podcasts(16,4,this.bettingOffset).subscribe(
      (res:any) => {
